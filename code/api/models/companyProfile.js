...
CompanyProfile.taskNreview = (req, result) => {
    ...
    var query = `SELECT userProfile.name as "reviewerName", userProfile.profilePicture as "reviewerPic", task.id as "taskID", task.typeID, task.title as "taskTitle", task.description as "taskDesc", task.price as "taskPrice", task.datePosted, task.dateCompleted, review.id as "reviewID", review.rating as "reviewRating", review.description as "reviewDesc"
                    FROM task
